...
import { Document, Page } from 'react-pdf'; // No need for pdfjs
import {
  ChevronLeft,
  ChevronRight,
  File,
...
const EBulletin = ({ activeScreen, languageType = 'en' }) => {
  ...
  // Adjust zoom on fullscreen toggle (unchanged)
  useEffect(() => {
    setScale(isFullScreen ? 1 : 0.8);
  }, [isFullScreen]);

  // Reset scroll on screen change (unchanged)
  useEffect(() => window.scrollTo(0, 0), [activeScreen]);

  // Fullscreen handlers (unchanged)
  ...
  // Toggle between one/two page view (unchanged)
      ...
      <div className={`e-bulletin-content ${isFullScreen ? 'fullscreen' : ''}`}>
        ...
        <div className="e-bulletin-pdf-container">
          {currentPdfUrl ? (
            ...
            >
              <div
                className={`e-bulletin-pages ${
                  isTwoPageView && !isMobile ? 'two-page-view' : 'one-page-view'
                }`}
              >
          ...
          {/* Fullscreen navigation overlays (unchanged) */}
          {isFullScreen && (
            <>
        ...
        {/* Mobile page input nav (unchanged) */}
